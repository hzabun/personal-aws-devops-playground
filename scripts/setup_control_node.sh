...
control_node_public_ip=$(terraform output -state="$(dirname "$0")/../terraform/terraform.tfstate" -json instance_public_ips | jq -r '.[0]' | xargs)

# Extract private IP addresses
all_private_ips=()
while IFS= read -r line; do
  all_private_ips+=("$line")
done < <(terraform output -state="$(dirname "$0")/../terraform/terraform.tfstate" -json instance_private_ips | jq -r '.[]')
